 ...
 * @tparam A
 *   the type of the input to the rule
 * @tparam B
 *   the type of the output of the rule
 */
trait Rule[A, B]:
  /**
   ...
   * @param value
   *   the value to apply the rule to
   * @return
...
sealed trait JoinRule[A, B] extends Rule[A, B]:
   ...
   * @param other
   *   the other rule to join with
   * @tparam C
   *   the type of the output of the other rule
   * @return
   *   a new rule that represents the composition of this rule and the other rule
   */
...
sealed trait ConditionalRule[A] extends Rule[A, Boolean]:
   ...
   * @param other
   *   the other rule to combine with
   * @return
...
object Rule:
  ...
  given policyConv[A]: Conversion[A => Boolean, Policy[A]] with
     ...
     * @return
     *   a policy rule that represents the function
     */
  ...
  given baseConv[A, B]: Conversion[A => B, Base[A, B]] with
     ...
     *   the function to convert
     * @return
     *   a base rule that represents the function
     */
   ...
   * @param f
   *   the function to convert to a rule
   * @tparam A
   *   the type of the input to the function
   * @return
   *   a conditional rule that represents the function
   */
