...
class CoordinatorTest extends AnyWordSpecLike with BeforeAndAfterAll {
  ...
  "A Coordinator" must {
    ...
    "return a map of pages and their crawled status" in {
      ...
      coordinator ! Coordinator.CheckPages(List("http://www.google.com", "http://www.github.com"), probe.ref)
      probe.expectMessage(Coordinator.PagesChecked(Map("http://www.google.com" -> false, "http://www.github.com" -> false)))
    }

    "return an empty map when no pages are provided" in {
      val probe = testKit.createTestProbe[Coordinator.PagesChecked]()
      val coordinator = testKit.spawn(Coordinator())
      coordinator ! Coordinator.CheckPages(List.empty, probe.ref)
      probe.expectMessage(Coordinator.PagesChecked(Map.empty))
    }
    ...
    "return a map with true values for pages that have been crawled" in {
      val probe = testKit.createTestProbe[Coordinator.PagesChecked]()
      val coordinator = testKit.spawn(Coordinator())
      coordinator ! Coordinator.SetCrawledPages(List("http://www.google.com"))
      coordinator ! Coordinator.CheckPages(List("https://www.google.com", "http://www.github.com"), probe.ref)
      probe.expectMessage(Coordinator.PagesChecked(Map("https://www.google.com" -> true, "http://www.github.com" -> false)))
    }

    "return a map with true values for pages that have been checked" in {
      val probe = testKit.createTestProbe[Coordinator.PagesChecked]()
      val coordinator = testKit.spawn(Coordinator())
      coordinator ! Coordinator.CheckPages(List("http://www.google.com"), probe.ref)
      probe.expectMessage(Coordinator.PagesChecked(Map("http://www.google.com" -> false)))
      coordinator ! Coordinator.CheckPages(List("https://www.google.com", "http://www.github.com"), probe.ref)
      probe.expectMessage(Coordinator.PagesChecked(Map("https://www.google.com" -> true, "http://www.github.com" -> false)))
    }
    ...
    "not add invalid URLs to the crawled list" in {
      val probe = testKit.createTestProbe[List[String]]()
      val coordinator = testKit.spawn(Coordinator())
      coordinator ! Coordinator.SetCrawledPages(List("invalid_url"))
      coordinator ! Coordinator.GetCrawledPages(probe.ref)
